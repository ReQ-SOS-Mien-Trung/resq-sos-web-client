...
async function toJpegBlob(file: File): Promise<Blob> {
  ...
  // Max dimension to cap resolution for very large images
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Try quality 0.85 first; if still >9 MB drop to 0.7
      canvas.toBlob(
        (blob) => {
          if (!blob) return reject(new Error("Không thể chuyển đổi ảnh"));
          if (blob.size <= 9 * 1024 * 1024) return resolve(blob);
          // Second pass with lower quality
          canvas.toBlob(
            (blob2) => {
...
/**
 * Upload an image to Cloudinary and return both the secure URL and public_id.
 * Use this when you need the publicId for potential cleanup on downstream failure.
 */
export async function uploadImageToCloudinaryWithId(
  file: File,
  folder = "resq/avatars",
  assetFolder?: string,
...
export async function deleteCloudinaryRawFiles(
  ...
  try {
    await fetch("/api/cloudinary/delete", {
      ...
      body: JSON.stringify({ publicIds, resourceType: "raw" }),
    });
  } catch {
...
/**
 * Delete image files from Cloudinary via server-side proxy.
 * Silently catches errors – this is a best-effort cleanup.
 */
export async function deleteCloudinaryImages(
  ...
  try {
    ...
    console.warn("[Cloudinary Cleanup] Failed to delete images:", publicIds);
  }
}
