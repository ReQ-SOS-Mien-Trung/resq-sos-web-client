...
export async function GET(request: Request) {
  ...
  const requestedQ = searchParams.get("q")?.trim();
  const shouldReturnRaw = searchParams.get("raw") === "1";

  if (requestedQ) {
    try {
      const url = new URL("https://api.weatherapi.com/v1/current.json");
      url.searchParams.set("key", apiKey);
      url.searchParams.set("q", requestedQ);
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `WeatherAPI error (${res.status}) for ${requestedQ}: ${text || res.statusText}`,
        );
      }
